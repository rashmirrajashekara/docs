...
* Create the `hvs.env` installation answer file.
   ...
   #To allow registration of specific hosts. 
   REQUIRE_EK_CERT_FOR_HOST_PROVISION=<true/false> # If set to true, worker node EK certificate should be registered in HVS DB, for AIK provisioning step of TA. Default value is false
   VERIFY_QUOTE_FOR_HOST_REGISTRATION=<true/false> # If set to true, when the worker node is being registered to HVS, quote verification will be done. Default value is false
